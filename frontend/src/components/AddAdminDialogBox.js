...
const AddAdminDialogBox = ({ val, openDialog, handleCloseDialog }) => {
  const dispatch = useDispatch();
  const [imagePrev, setImagePrev] = useState();

  const [addData, setAddData] = useState({
    super_admin_id: val._id,
    name: "",
    email: "",
    image: "",
    dateOfBirth: "",
  ...
  const addSuperAdminData = () => {
    ...
    formData.append("super_admin_id", addData.super_admin_id);
    formData.append("email", addData.email);
    formData.append("dateOfBirth", addData.dateOfBirth);
    formData.append("gender", addData.gender);
    formData.append("admin_image", addData.image);

    dispatch(addAdmin(formData, addData));

    handleCloseDialog();
  };

  useEffect(() => {
    setAddData({ ...addData, super_admin_id: val._id });
  }, []);
...
export default AddAdminDialogBox;
